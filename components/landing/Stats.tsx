...
function StatTile({ stat, index }: { stat: StatCard; index: number }) {
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1], delay: index * 0.11 }}
      whileHover={{ y: -6 }}
      className="rounded-3xl bg-white p-7 text-center shadow-[0_16px_40px_rgba(2,6,23,0.12)] ring-1 ring-slate-100"
    >
...
export function Stats({ dict }: StatsProps) {
  const { why } = dict;
  return (
    <section id="about" className="relative mx-auto w-full max-w-7xl px-5 py-20 sm:px-8 lg:py-28">
      <motion.div
      ...
      >
        ...
        <div className="relative grid items-center gap-12 lg:grid-cols-2 lg:gap-14">
          ...
          <div>
            ...
            <p className="mt-5 max-w-lg text-[1.02rem] leading-[1.75] text-[#475569]">{why.description}</p>
            <Link
              href="/login"
